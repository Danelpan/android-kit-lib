                ...
                try {
                    ...
                    if (fileDescriptor != null) {
                        ...
                        if(0 != degrees){
                            ...
                            Bitmap result = KitBitmapUtils.getRotationBitmap(bitmap, degrees);
                            if(result != bitmap){
                            	bitmap.recycle();
                            }
                            bitmap = result;
        ...
        if (isMounted()) {
            try {
                ...
                if (file.isDirectory()) {
                    ...
                    int currentTime = Integer.valueOf(dataFormat.format(date));
                    int subFileLastModified = Integer.valueOf(dataFormat.format(new Date(file.lastModified())).toString());
                    int day = currentTime - subFileLastModified;
                    if (day > expired) {
                        file.delete();
                    }
